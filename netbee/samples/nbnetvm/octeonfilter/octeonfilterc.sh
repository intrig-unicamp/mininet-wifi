...
function usage {
echo
echo "Usage: octeonfilterc.sh [options]"
echo "Compiles a filter in netpdf syntax into an octeon executable, using the octeonc backend"
echo
echo "Options:"
...
OCTEONC_BACKEND=`$NETVMCOMPILER -backends | awk '/octeonc/ {print $1}' | awk 'NR==1'`
if [ -z $OCTEONC_BACKEND ]; then
    echo Did you build the octeonc backend in the libnbnetvm?
    echo You need to enable it before running this sample.
    exit 1
fi

# Clean
rm -rf *.push.c *.init.c octeonc_precompile.c octeonc_precompile.h
...
# Compile netil
if ! $NETVMCOMPILER -b $OCTEONC_BACKEND -p $OCTEON_PREFIX $NETIL_FILE; then
	echo Error: unable to compile netil to octeon C code.
	exit 1
fi
...
mips64-octeon-linux-gnu-gcc $OCTEON_CFLAGS \
	octeon_appmain.c octeonc_precompile.c *.push.c *.init.c \
	-L$NETBEE_ROOT/lib/ -lnbnetvm -lcvmx \
	-I$OCTEON_ROOT/target/include/ \
	-I./config/ \
	-I$NETBEE_ROOT/include/ \
	-I$NETBEE_ROOT/src/nbnetvm/arch/octeon/ \
