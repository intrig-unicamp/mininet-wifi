...
class testWalkthrough(unittest.TestCase):
    ...
    def testHandover(self):
        ...
        sleep(2)
        p.sendline('sta1 iw dev sta1-wlan0 info')
        p.expect('ssid-ap1')
        p.expect(self.prompt)
        sleep(8)
        p.sendline('sta1 iw dev sta1-wlan0 info')
    ...
    def testMutipleSSID(self):
        ...
        p.expect('ssid1')
        p.expect(self.prompt)
        p.sendline('sta2 iw dev sta2-wlan0 info')
        p.expect('ssid2')
        p.expect(self.prompt)
        p.sendline('sta3 iw dev sta3-wlan0 info')
        p.expect('ssid2')
        p.expect(self.prompt)
        p.sendline('sta4 iw dev sta4-wlan0 info')
        p.expect('ssid3')
        p.expect(self.prompt)
        p.sendline('sta5 iw dev sta5-wlan0 info')
        p.expect('ssid4')
        p.expect(self.prompt)
        p.sendline('exit')
        p.wait()
