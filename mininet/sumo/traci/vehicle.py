...
def getDrivingDistance(vehID, edgeID, pos, laneID=0):
    ...
    .
    """
    trace._beginMessage(tc.CMD_GET_VEHICLE_VARIABLE, tc.DISTANCE_REQUEST, vehID,
                        1 + 4 + 1 + 4 + len(edgeID) + 4 + 1 + 1)
...
def setStop(vehID, edgeID, pos=1., laneIndex=0, duration=2 ** 31 - 1):
    trace._beginMessage(tc.CMD_SET_VEHICLE_VARIABLE, tc.CMD_STOP, vehID,
    ...
    trace._message.string += struct.pack("!BdBBBi", tc.TYPE_DOUBLE, pos, tc.TYPE_BYTE, laneIndex, tc.TYPE_INTEGER,
                                         duration)
    trace._sendExact()
...
def changeLane(vehID, laneIndex, duration):
    ...
    trace._message.string += struct.pack("!BiBBBi", tc.TYPE_COMPOUND, 2, tc.TYPE_BYTE, laneIndex, tc.TYPE_INTEGER,
                                         duration)
    trace._sendExact()
...
def slowDown(vehID, speed, duration):
    ...
    trace._message.string += struct.pack("!BiBdBi", tc.TYPE_COMPOUND, 2, tc.TYPE_DOUBLE, speed, tc.TYPE_INTEGER,
                                         duration)
    trace._sendExact()
...
def setAdaptedTraveltime(vehID, begTime, endTime, edgeID, time):
    trace._beginMessage(tc.CMD_SET_VEHICLE_VARIABLE, tc.VAR_EDGE_TRAVELTIME, vehID,
...
def setEffort(vehID, begTime, endTime, edgeID, effort):
    trace._beginMessage(tc.CMD_SET_VEHICLE_VARIABLE, tc.VAR_EDGE_EFFORT, vehID,
...
def setColor(vehID, color):
    ...
    trace._message.string += struct.pack("!BBBBB", tc.TYPE_COLOR, int(color[0]), int(color[1]), int(color[2]),
                                         int(color[3]))
    trace._sendExact()
...
def moveToVTD(vehID, edgeID, lane, x, y):
    trace._beginMessage(tc.CMD_SET_VEHICLE_VARIABLE, tc.VAR_MOVE_TO_VTD, vehID,
                        1 + 4 + 1 + 4 + len(edgeID) + 1 + 4 + 1 + 8 + 1 + 8)
    trace._message.string += struct.pack("!Bi", tc.TYPE_COMPOUND, 4)
    trace._message.string += struct.pack("!Bi", tc.TYPE_STRING, len(edgeID)) + edgeID
