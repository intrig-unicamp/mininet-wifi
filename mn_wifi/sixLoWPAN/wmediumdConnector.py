...
class w_cst:
    ...
    SOCKET_PATH = '/var/run/wmediumd_802154.sock'
    LOG_PREFIX = 'wmediumd_802154:'
...
class WStarter(object):
    ...
    def start(self, **kwargs):
        ...
        if wmediumd_mode.mode == 4:
            raise Exception("Wrong wmediumd_802154 mode given")
        wm = subprocess.call(['which', 'wmediumd_802154'],
        ...
        if wm == 0:
            ...
            info('*** Wmediumd_802154 is being used, but it is not installed.\n' \
                 '*** Please install Wmediumd_802154 with sudo util/install.sh -l.\n')
            exit(1)
    ...
    def initialize(self, **kwargs):
        ...
        if wmediumd_mode.mode is not w_cst.SPECPROB_MODE:
            ...
            wmd_config = tempfile.NamedTemporaryFile(
                prefix='mn_wmd_802154_config_', suffix='.cfg', delete=False)
            WStarter.wmd_config_name = wmd_config.name
            debug("Name of wmediumd_802154 config: %s\n" % WStarter.wmd_config_name)
            configstr = 'ifaces:\n{\n\tids = [\n'
            intfref_id = 0
            for intfref in kwargs['intfrefs']:
                if intfref_id != 0:
        ...
        WStarter.wmd_logfile = tempfile.NamedTemporaryFile(prefix='mn_wmd_log_',
                                                           suffix='.log',
                                                           delete=True)
        debug("Name of wmediumd_802154 log: %s\n" % WStarter.wmd_logfile.name)
        #if self.is_managed:
        #    cmdline[0:0] = ["nohup"]
