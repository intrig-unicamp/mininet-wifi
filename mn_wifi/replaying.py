...
from mn_wifi.link import ConfigWirelessLink
    ...
    def throughput(self):
        ...
        while self.thread_._keep_alive:
            ...
            for sta in stations:
                if hasattr(sta, 'time'):
                    if time_ >= sta.time[0]:
                        ConfigWirelessLink.config_tc(sta.wintfs[0], sta.throughput[0], 0, 0)
                ...
                if hasattr(sta, 'time'):
                    if time_ >= sta.time[0]:
                        if sta.wintfs[0].associatedTo:
                            ...
                            ConfigWirelessLink.config_tc(sta.wintfs[0], bw, loss, latency)
                        del sta.bw[0]
                        del sta.loss[0]
                        del sta.latency[0]
                        del sta.time[0]
                ...
                if hasattr(sta, 'time'):
                    if time_ >= sta.time[0]:
                        ...
                        if ap:
                            ...
                            ConfigWirelessLink(sta.wintfs[0], dist)
                        del sta.rssi[0]
                        del sta.time[0]
                    if len(sta.time) == 0:
                        staList.remove(sta)
